...
const nombres: Record<Metodo, string> = {
  cuadrados: "Cuadrados Medios",
  lehmer: "Método de Lehmer",
  aditivo: "Congruencial Aditivo",
  multiplicativo: "Congruencial Multiplicativo",
  mixto: "Congruencial Mixto",
};
const iniciales: Record<Metodo, Record<string, number>> = {
  cuadrados: { x0: 5731 },
  lehmer: { x: 37, y: 19 },
  aditivo: { semillas: 4, m: 10 },
  multiplicativo: { x0: 37, a: 19, m: 1032 },
  mixto: { x0: 37, a: 19, c: 33, m: 1032 },
};
...
function App() {
  ...
  const cambiarMetodo = (nuevo: Metodo) => {
    setMetodo(nuevo);
    setValores({ ...iniciales[nuevo], n: valores.n || 10 });
    setSecuencia([]);
  };
  const cambiar = (clave: string, valor: number) => {
    setValores((actual) => ({ ...actual, [clave]: valor }));
    if (metodo === "aditivo" && clave === "semillas")
      setSemillas((actual) =>
        Array.from(
          { length: Math.max(2, Math.min(20, valor || 2)) },
          (_, i) => actual[i] ?? 0,
        ),
      );
  };
  const generar = () => {
    ...
    const resultado =
      metodo === "cuadrados"
        ? generarCuadradosMedios(valores.x0 || 0, n)
        : metodo === "lehmer"
          ? generarLehmer(valores.x || 0, valores.y || 0, n)
          : metodo === "aditivo"
            ? generarCongruencialAditivo(semillas, valores.m || 1, n)
            : metodo === "multiplicativo"
              ? generarCongruencialMultiplicativo(
                  valores.x0 || 0,
                  valores.a || 0,
                  valores.m || 1,
                  n,
                )
              : generarCongruencialMixto(
                  valores.x0 || 0,
                  valores.a || 0,
                  valores.c || 0,
                  valores.m || 1,
  ...
  const copiar = async () => {
    await navigator.clipboard?.writeText(
      secuencia
        .map((fila) => `${fila.i}\t${fila.xi}\t${fila.ri.toFixed(5)}`)
        .join("\n"),
    );
  };
  ...
  const parametros =
    metodo === "cuadrados" ? (
      <div className="fields">{campo("x0", "Semilla (X₀)")}</div>
    ) : metodo === "lehmer" ? (
      ...
      <>
        <div className="fields">
          {campo("semillas", "Cantidad de semillas")}
          {campo("m", "Módulo (m)")}
        </div>
        <div className="seed-list">
          {semillas.map((semilla, i) => (
            <label key={i}>
              <span>X{i - semillas.length + 1}</span>
              <input
                type="number"
                value={semilla}
                onChange={(e) =>
                  setSemillas(
                    semillas.map((actual, j) =>
                      j === i ? Number(e.target.value) : actual,
                    ),
                  )
                }
              />
      ...
      <div className="fields">
        {campo("x0", "Semilla (X₀)")}
        {campo("a", "Multiplicador (a)")}
        {metodo === "mixto" && campo("c", "Constante (c)")}
        {campo("m", "Módulo (m)")}
      </div>
    );
  return (
      ...
      <main>
        <section className="config card">
          <div className="section-heading">
            <h2>Configuración</h2>
            <p>Parámetros del modelo matemático y tamaño muestral.</p>
          </div>
          <form
            onSubmit={(e) => {
              e.preventDefault();
          ...
          >
            <label>
              <span>Método de generación</span>
              <select
                value={metodo}
                onChange={(e) => cambiarMetodo(e.target.value as Metodo)}
              >
            ...
            <label>
              <span>N (Cantidad a generar)</span>
              <input
                type="number"
                min="1"
                max="10000"
        ...
        <section className="results card">
          <div className="result-head">
            <div>
              <div className="badges">
                <b>{nombres[metodo]}</b>
                <b>N = {valores.n}</b>
                {valores.m && <b>m = {valores.m}</b>}
              </div>
              <p>
                Resultados normalizados generados en el intervalo uniforme [0,
                1).
              </p>
            </div>
          ...
          <div className="table-wrap">
            ...
            {secuencia.length === 0 && (
              <div className="empty">
                Configura los parámetros y genera una secuencia.
              </div>
            )}
          </div>
          <footer>
            <span>Mostrando {secuencia.length} iteraciones calculadas</span>
            <span className="valid">
              ● {secuencia.length ? "Secuencia válida" : "Sin resultados"}
            </span>
          </footer>
        </section>
      </main>
